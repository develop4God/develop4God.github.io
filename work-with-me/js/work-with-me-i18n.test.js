...
// work-with-me-i18n.js is a browser IIFE that, on load, checks
...
const filename = path.join(__dirname, 'work-with-me-i18n.js');
...
const { getNestedTranslation, applyTranslationsForTest } = global.window.WorkWithMeI18n;
...
test('getNestedTranslation: resolves a deeply nested key', () => {
    const translations = { proof: { devocionales: { title: 'Devotionals' } } };
    assert.equal(getNestedTranslation(translations, 'proof.devocionales.title'), 'Devotionals');
});
